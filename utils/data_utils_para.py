...
class Operator:
    ...
    def step(self, plan, children, constraint):
        ...
        assert name in ['Gather Merge', 'Bitmap Heap Scan', 'Materialize', 'Merge Join',
                        'Seq Scan', 'Nested Loop', 'Bitmap Index Scan', 'Hash Join', 'Index Scan', 'Gather', 'Sort', 'Hash','BitmapAnd']
        #{'Sort', 'Bitmap Index Scan', 'BitmapAnd', 'Index Scan', 'Nested Loop', 'Materialize', 'Merge Join', 'Bitmap Heap Scan', 'Seq Scan', 'Hash', 'Hash Join'}
        ...
        for f in filter[t][:]:  # constraint list
            ...
            if flag:
                for i, attr in enumerate(attrs):
                    ...
                    hyperrange[i] = (hyperrange[i][0], hyperrange[i][1])
